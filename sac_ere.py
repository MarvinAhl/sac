...
class PolicyNetwork(nn.Module):
    ...
    def forward(self, state, stochastic=True):
        ...
        if stochastic:
            ...
            # Second term is result of applying tanh to the actions (clipping ensures numerical stability)
            log_probs = (pi_dist.log_prob(unbound_actions) - \
                torch.log((1.0 - actions**2).clip(1e-08, 1.0))).sum(dim=-1, keepdim=True)
        else:
...
class ValueNetwork(nn.Module):
    ...
    def __init__(self, state, actions, hidden=(512, 512)):
        ...
        modules_a.append(nn.LeakyReLU(0.01))
        modules_b.append(nn.LeakyReLU(0.01))
...
class SAC:
    def __init__(self, state, actions, policy_hidden=(512, 512), value_hidden=(512, 512), gamma=0.99,
        ...
        self.target_entropy = -actions  # Used to update alpha, more like a minimum entropy constraint

        self.learning_rate = learning_rate

        # Random actions for better exploration at start of training
        self.warmup_episodes = warmup_episodes
        self.episode = 0
        self.warmup_a = warmup_a
        self.warmup_b = warmup_b
        self.warmup_actions = np.zeros(actions, dtype=np.float32)
    ...
    def act(self, state):
        ...
        First actions are selected by an Ornstein-Uhlenbeck-Process
        for better initial exploration.
        """
        # Warmup episodes
        if self.episode < self.warmup_episodes:
            self.episode += 1
            N = self.rng.normal(0.0, 1.0, size=self.actions)
            self.warmup_actions = self.warmup_actions - self.warmup_a*self.warmup_actions + self.warmup_b*N
            actions = np.tanh(self.warmup_actions)
    ...
    def train(self, steps):
        ...
        for k in range(steps):
            ...
            td_targets = rewards + self.gamma * (1.0 - terminals) * \
            ...
            # Entropy Temperature update (uses log_alpha instead of alpha to make sure alpha is always positive)
